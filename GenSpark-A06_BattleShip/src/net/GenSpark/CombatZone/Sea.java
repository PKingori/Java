...
public class Sea {
    ...
    static {
        ships = new WarShip[] {
                new WarShip("ARLEIGH, the Destroyer", Load.ARLEIGH),
                new WarShip("NIMITZ, the Nuclear-Powered Aircraft Carrier Ship", Load.NIMITZ),
                new WarShip("TICONDEROGA, the Missile Ship Cruiser", Load.TICONDEROGA),
                new WarShip("ZUMWALT, the HUP Destroyer", Load.ZUMWALT),
                new WarShip("HAMILTON, the US Ship Frigate", Load.HAMILTON)
        };
    }
    ...
    public IBattleField getField(int x, int y) {
        if(!isInsideSeaGrid(x, y)) {
            throw new IllegalArgumentException("Out of Range - Please try again: ");
        }
        return seaBoard[y][x];
    }

    public void printSeaBoard() {
        System.out.print("\t");
    ...
    private boolean isValidStartingPoint(Point from, int length, boolean horizontal) {
        ...
        if(!isInsideSeaGrid(x, y) ||
                (!isInsideSeaGrid(x + length,y) && horizontal) ||
                (!isInsideSeaGrid(x, y + length) && !horizontal)
        ) {
            return false;
        }
    ...
    private boolean isInsideSeaGrid(int x, int y){
